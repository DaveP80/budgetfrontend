...
function EditEntry() {
    ...
    async function getInfo() {
        await axios.get(`http://localhost:9000/transactions/${id}`).then(res => {if (res.data.hasOwnProperty('id')) setFormData(res.data)}).catch(e => console.log(e))
    }

    async function updateEntry() {
        await axios.put(`http://localhost:9000/transactions/${id}`, formData).then(res => { if (res.data.some(item => item.id === id)) navigate(`/transactions/${id}`)})
